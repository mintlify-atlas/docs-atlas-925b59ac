...
Invoices are the central document of Eme2App's billing system. Every invoice moves through a well-defined lifecycle — from an editable **borrador** draft, through **confirmada** (emitted), to one of three settled states: **cobrada** (fully collected), **parcialmente cobrada** (partially collected), or **anulada** (voided). An overdue confirmed invoice whose due dates have passed is flagged as **vencida**. The backend enforces strict transition rules at each step, preventing data loss and ensuring fiscal integrity throughout.
...
```
borrador ──► confirmada ──► cobrada
                  │         parcialmente cobrada
                  │         vencida
                  └──► anulada (+ factura espejo negativa)
```

| Estado | Meaning |
|---|---|
| `borrador` | Draft — fully editable, no due dates generated yet |
| `confirmada` | Emitted — due dates generated from `forma_pago`; limited edits allowed |
| `cobrada` | All vencimientos have been collected |
| `parcialmente cobrada` | At least one vencimiento collected, others still pending |
| `vencida` | One or more vencimientos are past their `fecha_vencimiento` and unpaid |
...
<Note>
To delete the rectificative invoice and restore the original to `confirmada`, simply delete the rectificative via `DELETE /api/facturas/:id`. The backend detects the `factura_origen_anulacion_id` link and restores the original automatically.
</Note>
...
| `DELETE` | `/api/facturas/:id` | Delete invoice (borrador only) |
