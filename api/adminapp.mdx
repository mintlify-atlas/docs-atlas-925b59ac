...
description: "REST API reference for Eme2App superadmin operations. Manage companies, cross-company users, user-company assignments, system email config, and AEAT certificates."
---
...
Create a new user at the platform level and assign them to a company in a single request. If a user with the given email already exists, the creation step is skipped and the user is assigned to the specified company (if not already assigned). `empresa_id` is required at creation time.
...
<ParamField body="empresa_id" type="string" required>
  UUID of the company to assign the user to. Required — a user cannot be created without an initial company assignment.
</ParamField>
