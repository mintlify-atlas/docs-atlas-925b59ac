...
description: "Authenticate with Eme2App via JWT Bearer tokens. Log in, retrieve the current user, select a company, change passwords, and manage password resets."
---
...
Resets the admin user credentials to `admin@empresa.com` / `admin123`. Requires a valid Bearer token belonging to a `superadmin` user.
